...
RAW_DATASET_DIR = BASE_DIR / "datasets" / "plant-disease" / "_raw" / "PlantVillage" / "PlantVillage"
...
def prepare_dataset() -> None:
    ...
    raw_class_dirs = sorted(path for path in RAW_DATASET_DIR.iterdir() if path.is_dir())

    for class_dir in raw_class_dirs:
        ...
        print(
            f"{class_name}: train={len(train_images)} val={len(val_images)} test={len(test_images)}"
        )
